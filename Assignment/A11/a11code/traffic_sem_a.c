...
struct Street {
  ...
  uthread_sem_t sem_cross;
  int num_cars;
  int num_waiting;
  int num_before_switch; // number of cars remaining to enter before switching traffic direction
  enum Direction dir;
...
void initializeStreet(void) {
  ...
  street->num_waiting = 0; // number of cars blocked during switch lock
  street->dir = EAST;
  street->dirQueue = EAST;
  street->lock = uthread_sem_create(1);
  street->switch_lock = uthread_sem_create(0); // temporarily locks car when street switching sides or car is different direction
  street->sem_cross = uthread_sem_create(3); // occupancy lock
}
...
void enterStreet (enum Direction g) {
  ...
  while (street->num_before_switch <= 0 || (g != street->dir)) {
	  if (g != street->dir) {
		  street->dirQueue = g;
	  }
	  street->num_waiting++;
  ...
  uthread_sem_signal(street->lock);
  uthread_sem_wait(street->sem_cross);
  uthread_sem_wait(street->lock);
  entryTicker++;
  street->num_cars++;
  
  //printf("entering, now with %d cars, %d until switch\n",street->num_cars, street->num_before_switch);
  uthread_sem_signal(street->lock);
}
...
void leaveStreet(void) {
  ...
  if (street->num_before_switch == 0 && street->num_cars == 0) {
	  switch_direction();
	  street->num_before_switch = 6;
	  for (unsigned i = 0; i < street->num_waiting; i++) {
  ...
  uthread_sem_signal(street->sem_cross);
  uthread_sem_signal(street->lock);
}
